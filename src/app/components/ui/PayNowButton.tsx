...
export function PayNowButton({ premiumAmount, delivererName = "Gig Worker", period = "week", planName = "Premium", onSuccess }: Props) {
    ...
    const handleGatewayPay = async () => {
        ...
        await openPayment({
            ...
            onSuccess: (paymentId) => {
                ...
                toast.success(`Razorpay payment successful. ID: ${paymentId}`);
    ...
    const handleDemoPay = () => {
        ...
        window.setTimeout(() => {
            ...
            toast.success(`Demo payment completed. ID: ${paymentId}`);
            if (onSuccess) onSuccess(paymentId);
        }, 600);
    };
    ...
    return (
        <div className="space-y-3">
            ...
            >
                ...
                {loadingGateway ? "Opening Razorpay..." : `Pay with Razorpay - ₹${premiumAmount}/${period}`}
            </button>
            ...
            >
                ...
                {loadingDemo ? "Completing test payment..." : `Use Demo Payment - ₹${premiumAmount}/${period}`}
            </button>

            {!isConfigured && (
                <p className="text-center text-xs text-amber-700">
                    Add `VITE_RAZORPAY_KEY_ID` and restart the app to enable the real Razorpay window.
                </p>
            )}
        </div>
    );
