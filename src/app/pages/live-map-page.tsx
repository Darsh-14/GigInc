...
export function LiveMapPage() {
  ...
  useEffect(() => {
    ...
    // Draw static risk zones
    riskZones.forEach((zone) => {
      const cfg = riskConfig[zone.risk as keyof typeof riskConfig];
      L.circle([zone.lat, zone.lng], {
        color: cfg.color, fillColor: cfg.fillColor,
        fillOpacity: 0.35, weight: 1.5, radius: cfg.radius,
      }).addTo(map).bindPopup(
        `<div style="min-width:160px">
          <b style="font-size:13px">${zone.area}</b><br/>
          <span style="background:${cfg.color};color:#fff;padding:1px 8px;border-radius:999px;font-size:11px;font-weight:600">${cfg.label}</span>
          <p style="font-size:12px;color:#555;margin:4px 0 0">${zone.reason}</p>
        </div>`, { maxWidth: 220 }
      );
    });
  ...
  const startTracking = () => {
    ...
    const riderIcon = L.divIcon({
      ...
      html: `<div style="
        ...
        background:#009AFD;border:3px solid white;
        box-shadow:0 0 0 3px rgba(0,154,253,0.4),0 2px 8px rgba(0,0,0,0.3);
        animation:pulse-blue 1.5s infinite;">
      </div>
      <style>
        @keyframes pulse-blue {
          0%,100%{box-shadow:0 0 0 3px rgba(0,154,253,0.4),0 2px 8px rgba(0,0,0,0.3)}
          50%{box-shadow:0 0 0 8px rgba(0,154,253,0.15),0 2px 8px rgba(0,0,0,0.3)}
        }
      </style>`,
      iconSize: [18, 18],
      iconAnchor: [9, 9],
    ...
    watchId.current = navigator.geolocation.watchPosition(
      (pos) => {
        ...
        // Persist real GPS for fraud verification on claims page
        ...
        if (riderMark.current) {
          ...
          riderMark.current = L.marker([lat, lng], { icon: riderIcon, zIndexOffset: 1000 })
            .addTo(map)
            .bindPopup(`<b>Your live location</b><br/>Accuracy: ±${Math.round(accuracy)}m`);
        }
        ...
        if (accuracyCircle.current) {
          ...
          accuracyCircle.current = L.circle([lat, lng], {
            radius: accuracy, color: "#009AFD",
            fillColor: "#93c5fd", fillOpacity: 0.2, weight: 1, dashArray: "4 4",
          }).addTo(map);
        }

        // Fly to rider on first fix
        if (!hasFlown.current) {
      ...
      (err) => {
        ...
        setErrorMsg(
          ...
          err.code === 2 ? "Position unavailable. Check your device GPS." :
  ...
  const riskBadgeColor = nearRisk === "high" ? "bg-red-100 text-red-700 border-red-200"
    : nearRisk === "medium" ? "bg-amber-100 text-amber-700 border-amber-200"
    ...
    <div className="p-6 space-y-5 max-w-5xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 flex items-center gap-2">
            <Navigation className="w-7 h-7 text-brand-500" /> Live Delivery Tracker
          </h1>
          <p className="text-gray-500 mt-1 text-sm">
            Real-time GPS positioning + nearest insurance risk zone overlay
          </p>
        </div>
        <div className="flex gap-2">
          {status === "tracking" && (
            <Button variant="outline" onClick={recenter} className="border-brand-200 text-brand-700 hover:bg-brand-50">
              <Crosshair className="w-4 h-4 mr-1" /> Recenter
            </Button>
          )}
          {status !== "tracking" ? (
            <Button onClick={startTracking} className="bg-brand-500 hover:bg-brand-600 shadow-md">
              <MapPin className="w-4 h-4 mr-2" /> Start Live Tracking
            </Button>
          ) : (
            <Button onClick={stopTracking} variant="destructive" className="shadow-md">
              <WifiOff className="w-4 h-4 mr-2" /> Stop Tracking
            </Button>
          )}
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-3">
        <Badge className={`px-3 py-1.5 text-sm font-medium ${
          status === "tracking" ? "bg-green-100 text-green-700 border border-green-200" :
          status === "error"    ? "bg-red-100 text-red-700 border border-red-200" :
                                  "bg-gray-100 text-gray-600 border border-gray-200"
        }`}>
          {status === "tracking" ? <><Wifi className="w-3.5 h-3.5 mr-1 inline" /> Live — {pingCount} GPS pings</> :
           status === "error"    ? <><AlertTriangle className="w-3.5 h-3.5 mr-1 inline" /> Error</> :
                                   "Tracking off"}
        </Badge>
        {coords && (
          <>
            <Badge className="bg-brand-50 text-brand-700 border border-brand-200 px-3 py-1.5 text-sm font-mono">
              {coords.lat.toFixed(5)}, {coords.lng.toFixed(5)}
            </Badge>
            <Badge className="bg-slate-100 text-slate-600 border border-slate-200 px-3 py-1.5 text-sm">
        ...
        {nearestZone && nearRisk && (
          <Badge className={`px-3 py-1.5 text-sm border ${riskBadgeColor}`}>
            ...
            {riskConfig[nearRisk].label} zone — {nearestZone.distanceKm} km away ({nearestZone.zone.area})
          </Badge>
        )}
      </div>
      ...
      <Card className="overflow-hidden border-gray-200 shadow-md">
        <CardHeader className="pb-3 pt-4 px-5 bg-white border-b">
          <CardTitle className="text-base font-semibold text-gray-800 flex items-center gap-2">
            <MapPin className="w-4 h-4 text-brand-500" /> Live Map
            {status === "tracking" && (
              <RefreshCw className="w-3.5 h-3.5 ml-1 text-brand-400 animate-spin" />
            )}
            <span className="ml-auto text-xs font-normal text-gray-400">
              Colored zones = insurance risk areas · Blue dot = you
            </span>
          </CardTitle>
        </CardHeader>
      ...
      {status === "idle" && (
        <div className="text-center py-8 text-gray-500">
          ...
          <p className="font-medium text-gray-700">Start tracking to see your live position on the map</p>
          <p className="text-sm mt-1">Your browser will ask for location permission — click Allow</p>
        </div>
      )}
      ...
      <Card className="border-gray-100">
        <CardContent className="pt-4 pb-4">
          <p className="text-xs text-gray-500 uppercase tracking-wide font-semibold mb-3">Risk Zone Legend</p>
          ...
          <p className="text-xs text-gray-400 mt-3">
            Risk zones are updated based on historical weather, AQI, and flood data across major Indian cities.
